  ...
  #posts
    ...
    .flex.flex-wrap.mt-10
      a.relative.text-center.p-2.w-full(class="md:w-1/2" :href="post.attributes.url" v-for="post in posts" :key="post.attributes.title" target="_blank")
        ...
        img.lazyload.w-full.rounded(:src="'~/assets' + post.attributes.image" :srcset="require('~/assets' + post.attributes.image).srcSet" sizes="(min-width: 768px) 60vw, 95vw" v-if="post.attributes.image")
